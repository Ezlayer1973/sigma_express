...
def d_listararticulosapp(connection):
    if connection:
        try:
            cursor = connection.cursor()
            cursor.execute("SELECT * FROM articulos order by codigo;")            
            v_tabla = cursor.fetchall()
            total_articulos = len(v_tabla)
            cursor.close()
            print("Total de articulos: ", total_articulos)
            return v_tabla
        except (Exception, psycopg2.Error) as ex:
            print("Error al consultar a la Base de Datos", ex)

def d_unarticuloappcodigo(connection, v_parametros):
    if connection:
        try:
            cursor = connection.cursor()                
            v_texto = "SELECT id,codigo FROM articulos WHERE baja = False and id = %s;"                               
            ...
            print("Error al consultar a la Base de Datos de articulos Online", ex)
...
def d_insertararticuloapp(connection, v_parametros):
    if connection:
        try:
            cursor = connection.cursor()            
            v_texto = """
                INSERT INTO articulos (codigo,descri)
                VALUES (%s, %s)
                RETURNING id; 
            """                        
...
def d_eliminararticuloapp(connection, v_codigo):
    if connection:
        try:
            cursor = connection.cursor()
